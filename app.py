...
def distribute_lead():
    ...
    if region in regions and regions[region]:
        # Verifica se algum consultor da região atingiu o limite de leads
        if region == "TOPBR":
            available_consultors = [
                consultor for consultor in regions[region]
                if consultor_data[consultor]["leads_received"] < LEAD_LIMIT
            ]
            if not available_consultors:
                return jsonify(success=False, message="Limite de leads atingido para a região TOPBR.")
